...
FluContentPage {
    ...
    Component.onCompleted: {
        // 菜单列表
        root.menuOptions = MenuSlot.treeselect().data
        root.menuOptions = JSTools.renameKeys(root.menuOptions, 'id', 'key')
        root.menuOptions = JSTools.renameKeys(root.menuOptions, 'label', 'title')

        console.log(JSON.stringify(root.menuOptions))
        ...
        FluFrame {
            ...
            RowLayout {
                ...
                FluFilledButton {
                    text: "搜索"
                    onClicked: {
                        root.queryParams['roleName'] = query_roleName.text
                        root.queryParams['roleKey'] = query_roleKey.text
                        root.queryParams['status'] = JSTools.getStatus()[query_status.currentIndex]['value']
                        initData()
    ...
    function initData() {
        // 查询方法
        var result = RoleSlot.listRole(root.queryParams)
        // 处理查询结果
        dataSource = []
        for(var i in result.rows){
            dataSource.push({
                checkbox: table_view.customItem(com_checbox,{checked:false}),
                roleId: String(result.rows[i].roleId),
                roleName: String(result.rows[i].roleName),
                roleKey: String(result.rows[i].roleKey),
                roleSort: String(result.rows[i].roleSort),
